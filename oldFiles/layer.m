classdef layer < handle
    ...
    methods
        ...
        function coeff = getCoef(this, order, taoMult)
            coeff = this.mat(order+taoMult,order+1);
        end
        
        function setC(this, order, taoMult, value)
            this.mat(order+taoMult,order+1) = value;
        end
...
%             end
            s = size(this.mat);
            newMat = this.mat * diag(1 ./ (1:s(2)));
%           Integrate transition matrix
            newMat = [zeros(1, s(2)+1) ; [zeros(s(1),1) newMat]];
            new = layer(newMat);
        end
        ...
        function new = shift(this, d)
            newMat = this.mat * expandTransMat(size(this.mat, 2), d);
            new = layer(newMat);
%             s = size(this.mat, 1);
%             if s == 0
%                 new = layer([]);
