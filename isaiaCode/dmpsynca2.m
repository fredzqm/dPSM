...
%     SV = zeros(N,m+1); SV2 = zeros(N^2,m+1); SV3 = zeros(N^2,m+1);
    ...
    for i = 1:N
        ii = (i-1)*N;
        for j = 1:N
            t = calc(T(j,1:2), -1*tau, 0) - calc(T(i,1:2),0, 0);
            U(j+ii,1) = sin(t); 
            V(j+ii,1) = cos(t);
        end
    end
...
%             end
        ...
        for j= 1:N
            T(j,:) = [double(calc(T(j,1:ord+1), tau, 0)) zeros(1,ord)];
        end
        for i = 1:N
            ii = (i-1)*N;
            for j = 1:N
                U(j+ii,:) = [double(calc(U(j+ii,1:ord+1), tau, 0)) zeros(1,ord)];
                V(j+ii,:) = [double(calc(V(j+ii,1:ord+1), tau, 0)) zeros(1,ord)];
            end
        end
    end
    for i = 1:N-1
